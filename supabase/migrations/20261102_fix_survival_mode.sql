-- Migration: Fix Survival Mode Lives Logic
-- This updates the submit_answer function to properly check the configured survivalLives
-- before eliminating a participant.
...
DECLARE
  ...
  v_survival_lives INT;
  v_wrong_answers_count INT;
BEGIN
  -- 1. Validar estado de la sesión y configuración
  SELECT status, current_question_id, current_question_started_at, game_mode, config
  INTO v_session_status, v_current_question_id, v_started_at, v_game_mode, v_config
  ...
  -- 6. Lógica de Supervivencia: Contar errores antes de eliminar
  IF v_game_mode = 'survival' AND NOT v_is_correct THEN
    -- Obtenemos las vidas permitidas desde la config del profe (por defecto 1)
    v_survival_lives := COALESCE((v_config->>'survivalLives')::INT, 1);
    
    -- Contamos cuántos errores lleva en esta sesión ESPECÍFICA antes de que registre este nuevo
    SELECT COUNT(*) INTO v_wrong_answers_count
    FROM public.answers 
    WHERE participant_id = p_participant_id 
      AND session_id = p_session_id 
      AND is_correct = false;

    -- Si los errores viejos (v_wrong_answers_count) + este nuevo error (1) alcanzan o superan las vidas: Eliminado
    IF (v_wrong_answers_count + 1) >= v_survival_lives THEN
