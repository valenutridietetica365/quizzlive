...
export default function MultipleChoiceRenderer({ 
    ...
    return (
        <div className={`grid gap-2 md:gap-3 w-full ${currentQuestion.question_type === "true_false" ? "grid-cols-2" : "grid-cols-1 sm:grid-cols-2"}`}>
            {currentQuestion.options.map((opt: string, i: number) => {
                ...
                return (
                    <button
                        ...
                        className={`group p-3 md:p-6 rounded-xl md:rounded-2xl text-left transition-all active:scale-95 shadow-md border-b-[3px] flex flex-row items-center md:flex-col md:items-start md:justify-between min-h-[64px] md:h-auto overflow-hidden relative animate-in fade-in slide-in-from-bottom-8 fill-mode-both ${(isSubmitting || answered) && selectedOption !== opt ? "opacity-50 grayscale" : ""} ${selectedOption === opt ? "scale-[1.02] md:scale-105 brightness-110 z-10 ring-2 ring-white shadow-xl" : ""
                    ...
                    >
                        ...
                        {isSpyActive && count > 0 && (
                            <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded-full text-[10px] font-black text-slate-900 shadow-sm animate-in zoom-in duration-300">
                                👀 {percent}%
                            </div>
                        )}
